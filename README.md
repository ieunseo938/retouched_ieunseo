...
<head>
    ...
    <title>RETOUCHED: Floating Archive</title>
    
    <!-- 1. 폰트 해결: 파일 필요 없는 웹폰트(고운바탕) 로딩 -->
    ...
    <style>
        ...
        /* 사용법 안내 스타일 */
        .usage-guide { margin-top: 40px; opacity: 1; }
        .usage-title { font-weight: bold; font-size: 1rem; margin-bottom: 10px; display: block; color: #ff0000; }
        .usage-list { padding: 0; margin: 0; }
        .usage-list li { margin-bottom: 8px; word-break: keep-all; color: #ff0000; }

        /* 아이콘 범례 (빨강) */
        .icon-legend {
            display: flex; justify-content: flex-start; gap: 20px;
            max-width: 400px; margin-top: 30px; margin-bottom: 20px; pointer-events: auto;
        }
        .legend-item {
            display: flex; flex-direction: column; align-items: center;
            font-size: 0.8rem; color: #ff0000; opacity: 0.7; cursor: pointer;
            transition: transform 0.2s, opacity 0.2s;
        }
        .legend-item:hover, .legend-item.active { opacity: 1; transform: scale(1.1); font-weight: normal; }
        ...
        /* 카드 스타일 */
        .world { position: relative; width: 0; height: 0; transform-style: preserve-3d; display: flex; justify-content: center; align-items: center; z-index: 10; }
        .card {
            position: absolute; border-radius: 3px;
            transform: translate3d(0, 0, 0) rotate(0deg);
            transition: transform 1.8s cubic-bezier(0.2, 0.8, 0.2, 1), width 0.3s, height 0.3s, opacity 0.5s;
...
<body>
    ...
    <div class="overlay-text">
        <h1>RETOUCHED</h1>
        <p>
            '포토샵에 의한 죽음'. 디지털 픽셀이 발명되기 훨씬 전부터, 권력은 이미 <span class="highlight">'포토샵'</span>을 하고 있었다. 
            ...
            try {
                ...
                if (typeof Hands !== 'undefined' && typeof Camera !== 'undefined') {
                    ...
                    function onResults(results) {
                        cameraStatus.innerText = "Camera Active - Hand Control Ready";
                    ...
                    camera.start().catch(err => {
                        ...
                        cameraStatus.innerText = "Camera Error or Denied";
                    });
                } else {
                    console.log("MediaPipe not loaded in this environment.");
                }
